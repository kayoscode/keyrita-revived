...
namespace wgui
{
   ...
   public:
      ...
      value_type operator*() const { return mCurrent->get(); }
      value_type operator->() const { return mCurrent->get(); }
   ...
   public:
      ...
      static void GetControlsWithType(const __ControlIterator begin, const __ControlIterator end, std::vector<T*>& foundControls)
         requires std::is_base_of_v<GuiControlBase, T> &&
                  std::_Is_iterator_v<__ControlIterator>
      {
      ...
      static void GetControlsWithTag(const __ControlIterator begin, const __ControlIterator end, 
                                   const std::string& tag, std::vector<T*>& foundControls)
         requires std::is_base_of_v<GuiControlBase, T>&&
                  std::_Is_iterator_v<__ControlIterator>
      {
         for (auto it = begin; it != end; ++it)
      ...
      template <typename __AttrType, typename __AttrUnderlyingType,
                typename T = GuiControlBase, typename __ControlIterator = OwnedControlsIterator>
         static void GetControlsWithAttributeValue(const __ControlIterator begin, const __ControlIterator end,
            ...
            std::vector<T*>& foundControls)
         requires std::is_base_of_v<GuiControlBase, T> &&
                  std::is_base_of_v<CtrlAttribute, __AttrType> &&
                  EqualityComparable<__AttrUnderlyingType> &&
                  std::_Is_iterator_v<__ControlIterator>
      {
         for (auto it = begin; it != end; ++it)
         {
            ...
            if (val != nullptr && (*it)->GetAttributes()->AttributeExists(attributeName))
            {
               ...
               if (attr->Is<__AttrType>())
               {
                  ...
                  static_assert("Underlying attribute type must match the argument type" && std::is_same_v<decltype(attrValue->Get()), __AttrUnderlyingType>);
