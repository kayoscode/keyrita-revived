...
class AttrTypes
{
 public:
   ...
   inline static attr_type_id_t Int()
   {
      return IntId;
   }
   inline static attr_type_id_t Real()
   {
      return RealId;
   }
   inline static attr_type_id_t Bool()
   {
      return BoolId;
   }
   inline static attr_type_id_t Str()
   {
      return StrId;
   }
   inline static attr_type_id_t WinFlags()
   {
      return WinFlagsId;
   }
   inline static attr_type_id_t AlignFlags()
   {
      return AlignFlagsId;
   }
...
{
 public:
   AttrInt() : mValue(0), CtrlAttribute(AttrTypes::Int())
   {
   }
...
{
 public:
   AttrReal() : mValue(0.0), CtrlAttribute(AttrTypes::Real())
   {
   }
...
{
 public:
   AttrString() : CtrlAttribute(AttrTypes::Str())
   {
   }
...
{
 public:
   AttrBool() : mValue(false), CtrlAttribute(AttrTypes::Bool())
   {
   }
...
class AttributeSet
{
 ...
 private:
   std::map<std::string, std::unique_ptr<Attribute>, CaseInsensitiveStrCompare> mAttributes;
};
...
class AttributeParser
{
 public:
   ...
   /// <param name="text"></param>
   /// <param name="typeHint"></param>
   std::unique_ptr<Attribute> ParseAttribute(const std::string &text,
                                             attr_type_id_t typeHint = AttributeTypeManager::NotAType)
   {
      ...
      {
         std::unique_ptr<Attribute> parsedAttr =
            mAttributeFactories[i]->TryParseAttribute(text, typeHint, mTypeManager);
         ...
         if (parsedAttr != nullptr)
         {
            ...
            else if (mTypeManager.GetTypePriority(parsedAttr->GetType()) > lastParsedPriority ||
                     lastAttribute == nullptr)
            {
