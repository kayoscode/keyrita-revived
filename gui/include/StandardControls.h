...
class ControlTreeIterator
{
 public:
   ...
   {
      return (const reference)mCurrent;
   }
   pointer operator->()
   {
      return &mCurrent;
...
class GuiControlBase
{
 public:
   ...
   virtual float GetHeight(WindowBase *const window, nk_context *context) const
   {
      return 0;
   }
   ...
   virtual bool AddChild(GuiControlBase *control)
   {
      return false;
   }
      ...
      requires std::is_base_of_v<CtrlAttribute, Q>
   {
      ...
      {
         Application::Logger.warning("Overwriting previously defined attribute: '{str}' - incorrect type",
                                     attrName.c_str());
   ...
   AttributeSet *const GetAttributes()
   {
   ...
   ControlTreeIterator begin()
   {
      return ControlTreeIterator(this);
   }
   ControlTreeIterator end()
   {
      return ControlTreeIterator(nullptr);
   }
...
class ChildSupportingGuiControlBase : public GuiControlBase
{
 public:
   ...
   bool SupportsChildren() const override
   {
      return true;
   }
...
class GuiHorizontalSeparator : public GuiWidget
{
 public:
   ...
   GuiHorizontalSeparator()
      : mThickness(mAttributes->Add<AttrInt>((std::string)ThicknessAttribute)->GetRef()), GuiWidget()
   {
      mThickness = 1;
   }

   GuiHorizontalSeparator(float thickness) : GuiHorizontalSeparator()
   {
...
class GuiLabel : public GuiWidget
{
 public:
   ...
   GuiLabel()
      : GuiWidget(), mText(mAttributes->Add<AttrString>((std::string)TextAttr)->GetRef()),
   ...
   GuiLabel(const std::string &text, eTextAlignmentFlags alignment) : GuiLabel()
   {
      mText = text;
 ...
 public:
   ...
   GuiCheckbox()
      : mChecked(mAttributes->Add<AttrBool>((std::string)CheckedAttr)->GetRef()),
        mText(mAttributes->Add<AttrString>((std::string)GuiLabel::TextAttr)->GetRef()), GuiWidget()
   {
   }

   GuiCheckbox(const std::string &text, bool checked = false) : GuiCheckbox()
   {
      mChecked = checked;
      mText = text;
   }
...
{
 public:
   GuiButton() : mText(mAttributes->Add<AttrString>((std::string)GuiLabel::TextAttr)->GetRef())
   {
      mText = "";
   }

   GuiButton(const std::string &text) : GuiButton()
   {
      mText = text;
   }
...
class GuiRadioButton : public GuiWidget
{
 public:
   ...
   GuiRadioButton(const std::string &text, bool checked = false) : GuiRadioButton()
   {
      mText = text;
   }
...
{
 public:
   GuiComboboxItemBase() : mComboboxSelectedItem(nullptr), mItemHeight(nullptr)
   {
   }
...
class GuiComboboxItem : public GuiComboboxItemBase
{
 public:
   GuiComboboxItem()
      : GuiComboboxItemBase(), mText(mAttributes->Add<AttrString>((std::string)GuiLabel::TextAttr)->GetRef()),
   ...
   GuiComboboxItem(const std::string &text, eTextAlignmentFlags alignment) : GuiComboboxItem()
   {
      mText = text;
...
class GuiCombobox : public ChildSupportingGuiControlBase
{
 public:
   ...
   GuiCombobox(int selectedItem, int width = 200, int height = 200, int itemHeight = 35) : GuiCombobox()
   {
...
class GuiSliderInt : public GuiSlider
{
 public:
   ...
   GuiSliderInt(int64_t min, int64_t max, int64_t value, int64_t step = 1) : GuiSliderInt()
   {
      mValue = value;
      mMinValue = min;
      mMaxValue = max;
...
class GuiSliderReal : public GuiSlider
{
 public:
   ...
   GuiSliderReal(double min, double max, double value, double step = 1) : GuiSliderReal()
   {
      mValue = value;
      mMinValue = min;
      mMaxValue = max;
...
class GuiProgressBar : public GuiWidget
{
 public:
   ...
   GuiProgressBar(int64_t value, int64_t maxValue, bool modifiable = false) : GuiProgressBar()
   {
      mValue = value;
      mMaxValue = maxValue;
...
class GuiSelectableLabel : public GuiLabel
{
 public:
   ...
   GuiSelectableLabel(bool selected = false)
      : mSelected(mAttributes->Add<AttrBool>((std::string)SelectedAttr)->GetRef()), GuiLabel()
   {
      mSelected = selected;
   }
...
class GuiLayoutRowBase : public ChildSupportingGuiControlBase
{
 public:
   ...
   GuiLayoutRowBase(int height, bool autoHeight = false) : GuiLayoutRowBase()
   {
      mHeight = height;
   ...
   GuiLayoutRowBase()
      : ChildSupportingGuiControlBase(), mHeight(mAttributes->Add<AttrInt>((std::string)HeightAttr)->GetRef()),
...
class GuiLayoutRowStatic : public GuiLayoutRowBase
{
 public:
   ...
   GuiLayoutRowStatic(int height, int colWidth) : GuiLayoutRowStatic()
   {
      mColWidth = colWidth;
      mHeight = height;
   }

   GuiLayoutRowStatic() : mColWidth(mAttributes->Add<AttrInt>((std::string)ColWidthAttr)->GetRef()), GuiLayoutRowBase()
   {
      mColWidth = 100;
   }
...
class GuiLayoutRowDynamicGrid : public GuiLayoutRowBase
{
 public:
   ...
   GuiLayoutRowDynamicGrid() : GuiLayoutRowBase()
   {
   }
...
class GuiLayoutRowStaticGrid : public GuiLayoutRowBase
{
 public:
   ...
   GuiLayoutRowStaticGrid() : GuiLayoutRowBase()
   {
   }
 ...
 public:
   ...
   GuiLayoutTreeBase()
      : mInitiallyOpen(mAttributes->Add<AttrBool>((std::string)InitiallyOpenAttr)->GetRef()),
        mText(mAttributes->Add<AttrString>((std::string)GuiLabel::TextAttr)->GetRef()), mName(), mExpanded(false)
   {
      mText = "";
   ...
   GuiLayoutTreeBase(const std::string &text, bool initiallyOpen = false) : GuiLayoutTreeBase()
   {
...
{
 public:
   GuiLayoutTreeNode() : GuiLayoutTreeBase()
   {
   }
   GuiLayoutTreeNode(const std::string &text, bool initiallyOpen = false) : GuiLayoutTreeBase(text, initiallyOpen)
   {
   }
...
{
 public:
   GuiLayoutTreeTab() : GuiLayoutTreeBase()
   {
   }
   GuiLayoutTreeTab(const std::string &text, bool initiallyOpen = false) : GuiLayoutTreeBase(text, initiallyOpen)
   {
   }
...
class GuiMenu : public ChildSupportingGuiControlBase
{
 public:
   ...
   GuiMenu(const std::string &text, const eTextAlignmentFlags alignment, int height, int width,
...
class GuiMenuItem : public GuiWidget
{
 public:
   ...
   GuiMenuItem()
      : GuiWidget(), mTextAlignFlags(mAttributes->Add<AttrAlignFlags>((std::string)GuiLabel::TextAlignAttr)->GetRef()),
        mText(mAttributes->Add<AttrString>((std::string)GuiLabel::TextAttr)->GetRef()),
        mImagePath(mAttributes->Add<AttrString>((std::string)ImagePathAttr)->GetRef())
   {
      mTextAlignFlags = static_cast<int>(eTextAlignmentFlags::CenterLeft);
   ...
   GuiMenuItem(const std::string &text, const eTextAlignmentFlags alignment, int height, int width,
               const std::string &imagePath = "")
      : GuiMenuItem()
   {
      mTextAlignFlags = static_cast<int>(alignment);
...
{
 public:
   GuiAbstractControl() : mOwnedControls()
   {
   }
...
class GuiLayoutWindow : public GuiLayoutGroup
{
 public:
   ...
   GuiLayoutWindow()
      : GuiLayoutGroup(), mPosX(mAttributes->Add<AttrInt>((std::string)XPosAttr)->GetRef()),
   ...
   GuiLayoutWindow(const std::string &name, int posX, int posY, int width, int height, const std::string &title)
      : GuiLayoutWindow()
   {
      mPosX = posX;
      mPosY = posY;
