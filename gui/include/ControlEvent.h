...
#pragma region Event Observers
   ...
   public:
      ...
      void UpdateData(WindowBase* const window, nk_context* context, 
         const struct nk_rect& widgetBounds)
      {
...
#pragma region Event dispatcher
   ...
   public:
      ...
      template<typename T>
      static void DispatchEvent(GuiControlBase* rootControl, 
         T eventData, const EventHandler<T>& eventHandler)
            requires std::is_base_of_v<EventData, T>
      {
