...
class GuiControlFactoryBase
{
 ...
 protected:
      ...
      requires std::is_base_of_v<GuiControlBase, T>
   {
      ...
      mFactory.emplace(ctrl->GetLabel(), []() -> std::unique_ptr<GuiControlBase> {
   ...
   std::map<std::string, std::function<std::unique_ptr<wgui::GuiControlBase>()>, CaseInsensitiveStrCompare> mFactory;
};
