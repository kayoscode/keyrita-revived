...
/// <returns></returns>
static bool ParseToFlags(const std::string &value, int &flagsValue, std::function<int(std::string &text)> flagParseFunc)
{
   flagsValue = 0;
...
void StandardAttributeTypesParseFactory::Init(AttributeTypeManager &typeManager)
{
   ...
   mAttributeCreationFuncs.emplace(
      std::make_pair(AttrTypes::Int(), [](const std::string &text) -> std::unique_ptr<Attribute> {
         int64_t value;
   ...
   mAttributeCreationFuncs.emplace(
      std::make_pair(AttrTypes::Real(), [](const std::string &text) -> std::unique_ptr<Attribute> {
         double dVal;
   ...
   mAttributeCreationFuncs.emplace(
      std::make_pair(AttrTypes::Bool(), [](const std::string &text) -> std::unique_ptr<Attribute> {
         bool bVal;
   ...
   mAttributeCreationFuncs.emplace(
      std::make_pair(AttrTypes::WinFlags(), [](const std::string &text) -> std::unique_ptr<Attribute> {
         int flagsValue;
   ...
   mAttributeCreationFuncs.emplace(
      std::make_pair(AttrTypes::AlignFlags(), [](const std::string &text) -> std::unique_ptr<Attribute> {
         int flagsValue;
