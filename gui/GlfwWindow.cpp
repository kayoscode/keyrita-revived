...
NuklearGlfwContextManager::NuklearGlfwContextManager() : mNkGlfw(std::make_unique<nk_glfw>()), mNkContext(nullptr)
{
...
bool MainWindow::CreateWindow(const std::string &title, int width, int height, bool resizable, bool visible,
                              bool decorated, bool fullScreen)
{
   ...
   mFont = nk_font_atlas_add_from_file(fontAtlas, "res/fonts/RockoFLF.ttf", fontHeight, &cfg);
...
bool Dialog::CreateWindow(const std::string &title, int width, int height, bool resizable, bool visible, bool decorated,
                          bool fullScreen)
{
   mWindow = glfwCreateWindow(width, height, title.c_str(), fullScreen ? glfwGetPrimaryMonitor() : nullptr, nullptr);
   glfwMakeContextCurrent(mWindow);
...
WindowBase::WindowBase() : mWindow(nullptr), mWindowInput(this), mWindowTitle(), mNkContext(), mFont(nullptr)
{
}
