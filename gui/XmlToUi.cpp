      ...
      if (parsedAttr->GetType() != typeHint)
      {
         if (attrExists)
         {
            Application::Logger.error("Overwriting attribute {str} - incorrect type {str}!={str} value='{str}'",
                                      ...
                                      attributeParser.GetTypeManager().GetTypeName(typeHint).c_str(), attr->value());
         }
         else
         {
...
void XmlToUiUtil::ConstructControls(const xml_object_range<xml_node_iterator> &doc, GuiControlBase *parent,
                                    ...
                                    std::vector<GuiControlBase *> &resultControls)
{
   ...
   for (xml_node_iterator ctrl = doc.begin(); ctrl != doc.end(); ++ctrl)
   {
      ...
      {
         Application::Logger.error("No factory definition to build a control of type '{str}'", ctrl->name());
         assert(false);
      }
   }
}
